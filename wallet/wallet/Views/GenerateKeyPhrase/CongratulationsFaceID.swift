...
struct CongratulationsFaceID: View {
    ...
    var body: some View {
        ZStack{
            ...
            VStack{
                Text("Congratulation! \nBiometric ID setup successfully".localized()).padding(.trailing,30).padding(.leading,30).foregroundColor(.white).multilineTextAlignment(.center).lineLimit(nil).font(.barlowRegular(size: Device.isLarge ? 36 : 28)).padding(.top,40)
